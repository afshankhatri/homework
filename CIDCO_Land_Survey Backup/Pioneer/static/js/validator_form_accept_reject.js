    ...
    fetch('/get_outputform_data')
        ...
        .then(data => {
            if (data.error) {
                ...
                document.getElementById("remarks").textContent = data.remarks || "No data";
...
document.getElementById("acceptValidationButton").addEventListener("click", () => {
    const plotdetailsUid = document.getElementById("surveyformdata_uid").value;
    ...
    if (confirm("Are you sure you want to accept?")) {
        ...
        fetch('/update_validation', {
            ...
            body: JSON.stringify({
                surveyformdata_uid: plotdetailsUid,
                is_validation_done: 1,
                survey_form_status: 2,
            }),
        })
        .then(response => response.json())
        .then(data => {
